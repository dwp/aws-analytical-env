...
resource "aws_wafregional_rule" "detect_large_file_uploads" {
  name        = "detect-large-file-uploads"
  metric_name = "largeFileUploads"

  predicate {
    data_id = aws_wafregional_byte_match_set.match_post_put.id
    negated = false
    type    = "ByteMatch"
  }

  predicate {
    data_id = aws_wafregional_size_constraint_set.file_upload_size_restrictions.id
    negated = false
    type    = "SizeConstraint"
  }
...
resource "aws_wafregional_rate_based_rule" "detect_sequential_file_uploads" {
  name        = "detect-sequential-file-uploads"
  metric_name = "sequentialFileUploads"

  rate_key   = "IP"
  rate_limit = var.file_upload_rate_limit

  predicate {
    data_id = aws_wafregional_byte_match_set.match_files_paths.id
    negated = false
    type    = "ByteMatch"
  }
