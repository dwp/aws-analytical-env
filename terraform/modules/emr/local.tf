locals {
  ...
  configurations_mysql_json = templatefile(format("%s/templates/emr/configuration.mysql.json", path.module), {
    ...
    hive_java_ops_xmx                   = local.hive_tez_container_size[var.environment] * 0.8
    tez_runtime_io_sort                 = local.hive_tez_container_size[var.environment] * 0.4
    tez_runtime_unordered_output_buffer = local.hive_tez_container_size[var.environment] * 0.1
  })
